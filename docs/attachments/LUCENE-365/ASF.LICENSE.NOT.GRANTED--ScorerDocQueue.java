 ...
 */
public class ScorerDocQueue {  // later: ScorerDocPosQueue for spans
  ...
  private class HeapedScorerDoc {
    ...
    HeapedScorerDoc(Scorer s, int docNr) {
      scorer = s;
      doc = docNr;
    }
    void adjust() {
  ...
  private HeapedScorerDoc topHSD; // same as heap[1]
  ...
  public boolean insert(Scorer scorer){
    if (size < maxSize) {
      ...
      /* This relies on lessThan() comparing document numbers as smaller: */
  ...
  public final boolean topNext() throws IOException {
    return topHSD.scorer.next();
  }
  ...
  private boolean checkAdjustElsePop(boolean cond) {
    if (cond) { // adjustTop inlined, without downHeap()
      topHSD.doc = topHSD.scorer.doc();
    } else { // popNoResult inlined, without downHeap()
