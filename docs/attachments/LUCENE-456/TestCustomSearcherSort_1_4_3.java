...
public class TestCustomSearcherSort_1_4_3
extends TestCase
implements Serializable {

    private Directory index = null;
    private Query query = null;
    private final static int INDEX_SIZE = 20000;

	public TestCustomSearcherSort_1_4_3 (String name) {
		super (name);
	}
	...
	public static Test suite() {
		return new TestSuite (TestCustomSearcherSort_1_4_3.class);
	}
	...
	throws IOException {
	    ...
	    for (int i=0; i<INDEX_SIZE; ++i) { // don't decrease; if to low the problem doesn't show up
	        ...
	        if((i%5)!=0) { // some documents must not have an entry in the first sort field
	            doc.add (new Field ("publicationDate_", random.getLuceneDate(), true, true, true)); // store, index, token
	        }
	        if((i%7)==0) { // some documents to match the query (see below) 
	            doc.add (new Field ("content", "test", true, true, true));
	        }
	        // every document has a defined 'mandant' field
	        doc.add(new Field("mandant", Integer.toString(i%3), true, true, true));
...
//        assertEquals(resultMap.size(), 0);
    ...
    public class CustomSearcher extends IndexSearcher {
        ...
        public TopFieldDocs search(Query query, Filter filter, int nDocs,
            ...
            bq.add(new TermQuery(new Term("mandant", Integer.toString(switcher))), true, false);
    ...
    private class RandomGen {
        private Random random = new Random(); // to generate some arbitrary contents
	    private Calendar base = new GregorianCalendar(1980, 1, 1);

	    // Just to generate some different Lucene Date strings
        private String getLuceneDate() {
    	    return DateField.timeToString(base.getTimeInMillis() + random.nextInt() - Integer.MIN_VALUE);
        }
    }
}
