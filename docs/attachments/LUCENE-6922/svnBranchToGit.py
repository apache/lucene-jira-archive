...
class SubProcessAtPath(object):
  ...
  def checkCall(self, *args, **kwArgs):
    ...
    subprocess.check_call(*args, **kwArgs) # CHECKME: encode strings to sequences?

  def checkOutput(self, *args, **kwArgs):
    self.chDirToPath()
    if self.verbose:
      print("check_output args:", " ".join(*args), str(**kwArgs))
    result = subprocess.check_output(*args, **kwArgs) # CHECKME: encode strings to sequences?
    if self.verbose:
...
class GitRepository(SubProcessAtPath):
  ...
  def getCurrentBranch(self):
    result = self.checkOutputAsStr((self.gitCmd, "symbolic-ref", "--short", "HEAD")).rstrip('\n')
    # print("getCurrentBranch: '%s'" % result)
    return result

  def workingDirectoryClean(self):
    gitStatusOut = self.checkOutputAsStr((self.gitCmd, "status", "--porcelain"))
    #print("workingDirectoryClean gitStatusOut='%s'" % gitStatusOut)
    return gitStatusOut == ""

  def branchExists(self, branchName):
    try:
      ignore = self.checkOutput((self.gitCmd, "rev-parse", "--verify", (branchName + "^{commit}")), stderr=subprocess.STDOUT)
      return True
    except subprocess.CalledProcessError, cpe:
      notExistExitValue = 128
      if cpe.returncode != notExistExitValue:
        assert cpe.returncode == notExistExitValue, str(cpe) + (", output='%s'" % cpe.output)
      return False
  ...
  gitSvnMarker = "git-svn-id:" # added and used by git svn dcommit, and used here.
  ...
  def commit(self, message,
    ...
    self.checkCall((self.gitCmd, "commit",
                                  ...
                                  ("--message=" + message),
                                  "--cleanup=verbatim", # no message cleanup
                                  ("--author=" + author),
                                  ("--date=" + authorDate) ))
...
def verifyGitFilesAgainstSvn(gitRepo, svnWorkingCopy):
  # The files under version control at the git repo can be enumerated quickly by: git ls-tree -r --name-only
  # This makes sense because all files, including binary files, are added.
  # svn ls -R  is too slow to use here (this lists about 220 file names per second, lucene-solr has over 9000).
  ...
  for svnUpdateLine in nonEmptyLines(svnUpdateOutputLines):
    ...
    if itemChar == "D": # deleted in svn working copy
      if os.path.isdir(fileNameInGitRepo):
        ...
        print("Not deleting not existing file %s" % fileName)
      ...
      if os.path.isdir(fileNameInSvnWorkingCopy):
        if not os.path.isdir(fileNameInGitRepo):
          ...
          os.mkdir(fileNameInGitRepo) # CHECKME: this might fail when a file was updated to a directory.
        else:
        ...
        if not os.path.isdir(head):
          ...
          os.mkdir(head) # CHECKME: ok when more than one level of directory needs to be created?
...
def maintainTempGitSvnBranch(branchName, tempGitBranchName,
  ...
  for (logEntryFrom, logEntryTo) in allSuccessivePairs(svnLogEntries):
    ...
    # commit, include toRevision so it can be picked up later.
...
if __name__ == "__main__":
  ...
  selfAbsPath = os.path.abspath(sys.argv[0])

  if selfAbsPath.startswith(svnWorkingCopiesPath) or selfAbsPath.startswith(gitReposPath):
    errorExit(selfAbsPath + " cannot run from svn working copy or git working tree, copy to another place and run from there.")
  ...
  """
  gitRepoPath2 = os.path.join(gitReposPath, repo + "-svn2git")
  gitUpstream2 = "origin"
  branchName2 = "master" # cannot merge origin/trunk
  tempGitBranchName2 = branchName2 + ".svn"

  maintainTempGitSvnBranch(branchName2, tempGitBranchName2,
                            svnWorkingCopyOfBranchPath, svnRepoBranchName,
                            gitRepoPath2, gitUpstream2,
                            maxCommits=maxCommits,
                            testMode=testMode)

  """

  branchName2 = "lucene_solr_5_3"
  tempGitBranchName2 = branchName2 + ".svn"
  svnRepoBranchName2 = "lucene/dev/branches/" + branchName2 # for svn switch to
  gitUpstream2 = "apache"

  maintainTempGitSvnBranch(branchName2, tempGitBranchName2,
                            svnWorkingCopyOfBranchPath, svnRepoBranchName2,
                            gitRepoPath, gitUpstream2,
                            maxCommits=maxCommits,
                            testMode=testMode)
