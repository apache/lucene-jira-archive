...
public class MultiFieldQueryParser extends QueryParser
{
  ...
  public void setBoostMap(Map boostMap){
	  this.boostMap=boostMap;
  }
  
  protected void boostQuery(String field,Query q){
	if (boostMap!=null && field!=null){
		Float boostVal=(Float)boostMap.get(field);
		if (boostVal!=null){
			float previousVal=q.getBoost();
			q.setBoost(previousVal*boostVal.floatValue());
		}
	}
  }
  
  protected Query getFieldQuery(String field, String queryText) throws ParseException {
    return getFieldQuery(field,null,queryText);
  }
  ...
  protected Query getFieldQuery(String field, Analyzer analyzer, String queryText)
    ...
    if (field == null) {
      ...
      for (int i = 0; i < fields.length; i++){
		  q=super.getFieldQuery(fields[i], queryText);
		  if (q!=null){
			  boostQuery(fields[i],q);
  ...
  protected Query getFuzzyQuery(String field, String termStr, float minSimilarity) throws ParseException
  {
    if (field == null) {
	  ...
	  Query q;
      for (int i = 0; i < fields.length; i++) {
		  q=super.getFuzzyQuery(fields[i], termStr, minSimilarity);
		  if (q!=null){
			  boostQuery(fields[i],q);
			  clauses.add(new BooleanClause(q,BooleanClause.Occur.SHOULD));
		  }
  ...
  protected Query getPrefixQuery(String field, String termStr) throws ParseException
  {
    if (field == null) {
	  ...
	  Query q;
      for (int i = 0; i < fields.length; i++) {
		  q=super.getPrefixQuery(fields[i], termStr);
		  if (q!=null){
			  boostQuery(fields[i],q);
			  clauses.add(new BooleanClause(q,BooleanClause.Occur.SHOULD));
		  }
  ...
  */
  ...
  protected Query getRangeQuery(String field, String part1, String part2, boolean inclusive) throws ParseException {
    if (field == null) {
	  ...
	  Query q;
      for (int i = 0; i < fields.length; i++) {
		  q=super.getRangeQuery(fields[i], part1, part2, inclusive);
		  if (q!=null){
			  boostQuery(fields[i],q);
			  clauses.add(new BooleanClause(q,BooleanClause.Occur.SHOULD));
		  }
