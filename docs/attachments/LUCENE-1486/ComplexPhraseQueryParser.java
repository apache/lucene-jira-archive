...
public class ComplexPhraseQueryParser extends QueryParser
{
			...
			boolean inclusive)
	{
		if (isPass2ResolvingPhrases)
		{
			...
			return new RangeQuery(new Term(field, part1),
					new Term(field, part2), inclusive, getRangeCollator());
		}
	...
	/*
	 * Used to handle the query content in between quotes and produced
	 * Span-based interpretations of the clauses.
	 */
	static class ComplexPhraseQuery extends Query
	{
		...
		private Term templateTerm;

		public ComplexPhraseQuery(String field,
				String phrasedQueryStringContents, int slopFactor)
		{
			super();
			this.field = field;
			this.phrasedQueryStringContents = phrasedQueryStringContents;
			this.slopFactor = slopFactor;
			templateTerm = new Term(field, "");
		}
		...
		public Query rewrite(IndexReader reader) throws IOException
		{
			...
			for (int i = 0; i < bclauses.length; i++)
			{
				...
				if (qc instanceof BooleanQuery)
				{
					...
					{
						//Insert fake term e.g. phrase query was for "Fred Smithe*" and there were no "Smithe*" terms - need to 
			...
			{
				if (!bclauses[j].getOccur()
						.equals(BooleanClause.Occur.MUST_NOT))
				{
			...
			} else
			{
				...
				// negatives
				include = new SpanNearQuery(includeClauses, slopFactor
						+ numNegatives, true);
			}
			// Use sequence of positive and negative values as the exclude.
			SpanNearQuery exclude = new SpanNearQuery(allSpanClauses,
					slopFactor, true);
