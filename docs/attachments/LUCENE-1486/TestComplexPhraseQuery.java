...
public class TestComplexPhraseQuery extends TestCase{
	...
	public void testComplexPhrases() throws Exception 
	{
		...
		checkMatches("\"(john OR johathon)  smith\"", "1,2");  // boolean logic with brackets works.
//		checkMatches("\"john -percival\"", "1");  // not logic doesn't work currently  :(.
		
		checkMatches("\"john  nosuchword*\"", "");  	// phrases with clauses producing empty sets
...
//	private static String highlightText(String fieldName, Analyzer a, Query q,
//			String text) throws IOException
//	{
//		CachingTokenFilter tokenStream = new CachingTokenFilter(a.tokenStream(
//		        fieldName, new StringReader(text)));
//		    // Assuming "<B>", "</B>" used to highlight
//		    SimpleHTMLFormatter formatter = new SimpleHTMLFormatter();
//		    Highlighter highlighter = new Highlighter(formatter, new SpanScorer(q, fieldName,
//		        tokenStream, "text"));
...
//
	...
	protected void setUp() throws Exception
	{
		...
		for (int i = 0; i < docsContent.length; i++)
		{
			...
			doc.add(new Field("name",docsContent[i].name,Field.Store.YES,Field.Index.ANALYZED));
			doc.add(new Field("id",docsContent[i].id,Field.Store.YES,Field.Index.ANALYZED));
			w.addDocument(doc);
		}
		w.close();
