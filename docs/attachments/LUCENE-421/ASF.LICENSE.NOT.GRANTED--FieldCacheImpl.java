...
class FieldCacheImpl implements FieldCache
{
    ...
    public int[] getInts(IndexReader reader, String field) throws IOException
    {
        ...
        if (ret == null)
        {
            ...
            int[] tmpdocs = new int[10000];
            int[] tmpfreqs = new int[10000];
            if (retArray.length > 0)
            {
                ...
                try
                {
                    if (termEnum.term() == null) { throw new RuntimeException(
                    ...
                    do
                    {
                        ...
                        int readcnt;
                        while ( (readcnt = termDocs.read(tmpdocs, tmpfreqs)) > 0) {
                            for (int j = 0; j < readcnt; ++j) {
                                retArray[tmpdocs[j]] = termval;
                            }
                        }
                        /*
                        while (termDocs.next())
    ...
    {
        field = field.intern();
        Object ret = lookup(reader, field, FieldCache.INTEGER_RANGE);
        if (ret == null)
        {
            int[] valArray = getInts(reader, field);
            final int[] retArray = new int[reader.maxDoc()];
            if (retArray.length > 0)
            {
                for (int ii = 0; ii != reader.maxDoc(); ++ii)
                {
                    retArray[ii] = ii;
                }
                Sort.quickSort(retArray, 0, retArray.length - 1, valArray);

            }
            store(reader, field, FieldCache.INTEGER_RANGE, retArray);
    ...
    {
        field = field.intern();
        Object ret = lookup(reader, field, FieldCache.FLOAT_RANGE);
        if (ret == null)
        {
            float[] valArray = getFloats(reader, field);
            final int[] retArray = new int[reader.maxDoc()];
            if (retArray.length > 0)
            {
                for (int ii = 0; ii != reader.maxDoc(); ++ii)
            ...
            store(reader, field, FieldCache.FLOAT_RANGE, retArray);
            return retArray;
        }
        return (int[]) ret;
    }
    ...
    public Object getAuto(IndexReader reader, String field) throws IOException
    {
        ...
        if (ret == null)
        {
            ...
            try
            {
                ...
                if (term == null) { throw new RuntimeException(
                        "no terms in field " + field
