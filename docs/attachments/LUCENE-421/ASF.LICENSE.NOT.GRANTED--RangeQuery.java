...
public class RangeQuery extends Query
{
    ...
    {
        if (lowerTerm == null && upperTerm == null) { throw new IllegalArgumentException(
                "At least one term must be non-null"); }
        if (lowerTerm != null && upperTerm != null
                && lowerTerm.field() != upperTerm.field()) { throw new IllegalArgumentException(
     ...
     * 
     * @param reader
     *            an <code>IndexReader</code> value
    ...
    public Query rewrite(IndexReader reader) throws IOException
    {
            ...
            do
            {
                ...
                if (term != null && term.field() == testField)
                {
                            ...
                            || term.text().compareTo(lowerTerm.text()) > 0)
                    {
                        ...
                        if (upperTerm != null)
                        {
                            int compare = upperTerm.text().compareTo(
                                    term.text());
                            /*
        ...
        catch (TooManyClauses tmc)
        {
            ...
            try
            {
                Integer lower;
                Integer upper;
                if (lowerTerm.text().length() != 0)
                    lower = new Integer(lowerTerm.text());
                else
                    lower = new Integer(Integer.MIN_VALUE);
                if (upperTerm.text().length() != 0)
                    upper = new Integer(upperTerm.text());
                else
                    upper = new Integer(Integer.MAX_VALUE);
                return new IntegerRangeQuery(lowerTerm.field(), lower, upper,
                        inclusive);
            ...
            try
            {
                ...
                Float upper;
                if (lowerTerm.text().length() != 0)
                    lower = new Float(lowerTerm.text());
                else
                    lower = new Float(Float.MIN_VALUE);
                if (upperTerm.text().length() != 0)
                    upper = new Float(upperTerm.text());
                else
                    upper = new Float(Float.MAX_VALUE);
                return new FloatRangeQuery(lowerTerm.field(), lower, upper,
                        inclusive);
