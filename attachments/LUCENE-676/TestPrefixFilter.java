...
public class TestPrefixFilter extends TestCase {
  public void testPrefixFilter() throws Exception {
    ...
    assertEquals("All documents in /Computers category and below", 4, hits.length());

    filter = new PrefixFilter(new Term("category", "/Computers"));
    query = new ConstantScoreQuery(filter);
    hits = searcher.search(query);
    assertEquals("One in /Computers/Mac", 2, hits.length());
