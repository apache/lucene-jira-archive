...
final class IntegerRangeScorer extends Scorer {
	...
	IntegerRangeScorer(Weight weight, int lowval, int hival, boolean inclusive,
		...
		if (this.docnos.length > 1)
			Sort.quickSort(this.docnos, 0, this.docnos.length - 1);
	}
	...
	public static int indexOfLeastUpperBound(int[] docnums, int[] docvals,
	...
	public static int indexOfGreatestLowerBound(int[] docnums, int[] docvals,
			int lo, int hi, int val) {
		while (hi >= lo) {
			int pivot = lo + (hi - lo)/2;
			if (val < docvals[docnums[pivot]]) {
