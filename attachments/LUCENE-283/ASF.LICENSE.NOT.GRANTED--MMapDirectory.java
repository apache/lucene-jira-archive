...
public class MMapDirectory extends FSDirectory {
  ...
  private class MultiMMapIndexInput extends IndexInput {
    ...
    public byte readByte() throws IOException {
      // Performance might be improved by reading ahead into an array of eg. 128 bytes
      // and readByte() from there.
      if (curAvail == 0) {
        curBufIndex++;
    ...
    public void readBytes(byte[] b, int offset, int len) throws IOException {
      while (len > curAvail) {
        ...
        curBuf.position(0); // index out of bounds when too many bytes requested
        curAvail = bufSizes[curBufIndex];
      }
    ...
    public Object clone() {
      ...
      // Since most clones will use only one buffer, duplicate() could also be done lazy in clones,
      // eg. when adapting curBuf.
  ...
  //private final int MAX_BBUF = 1023 * 129; // TestCompoundFile uses 1MB
