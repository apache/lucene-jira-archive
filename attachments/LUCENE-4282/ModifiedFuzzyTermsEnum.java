package de.insiders.isearch.searcher.fuzzy.query;

/*
 * Modification of the original class
...
/**
 * Modification: The boost of the query has been modified.<br>
 * Without this modification the test ISearchImplIndex2Test.testDoSearch_Fuzzy_1_5
 * will fail.<br>
 * 
 * Subclass of TermsEnum for enumerating all terms that are similar to the
 * specified filter term.
 * 
 * <p>
 * Term enumerations are always ordered by {@link #getComparator}. Each term in
 * the enumeration is greater than all that precede it.
 * </p>
 */
public class ModifiedFuzzyTermsEnum extends TermsEnum {
	private TermsEnum actualEnum;
	private BoostAttribute actualBoostAtt;

	private final BoostAttribute boostAtt = attributes().addAttribute(
	...
	private final Comparator<BytesRef> termComparator = BytesRef
	 ...
	 * <code>prefixLength</code> with <code>term</code> and which have a fuzzy
	 ...
	 *            Delivers terms.
	 * @param atts
	 *            {@link AttributeSource} created by the rewrite method of
	 *            {@link MultiTermQuery} thats contains information about
	 *            competitive boosts during rewrite. It is also used to cache
	 *            DFAs between segment transitions.
	 * @param term
	 *            Pattern term.
	 * @param minSimilarity
	 *            Minimum required similarity for terms from the reader. Pass an
	 *            integer value representing edit distance. Passing a fraction
	 *            is deprecated.
	 * @param prefixLength
	 *            Length of required common prefix. Default value is 0.
	 * @throws IOException
	 */
	public ModifiedFuzzyTermsEnum(Terms terms, AttributeSource atts, Term term,
			final float minSimilarity, final int prefixLength,
			boolean transpositions) throws IOException {
		...
		for (int cp, i = 0, j = 0; i < utf16.length(); i += Character
				.charCount(cp))
		...
		if (minSimilarity >= 1f) {
// USERCODE_BEGIN this seems to be wrong since 0.6 of 5 is 3, which means 2 edits
// and calculated min similarity was 0.4
//			this.minSimilarity = 1 - (minSimilarity + 1) / this.termLength;
			this.minSimilarity = 1 - (minSimilarity / this.termLength);
		...
		if (transpositions
			...
			throw new UnsupportedOperationException(
					"with transpositions enabled, distances > "
	...
	protected TermsEnum getAutomatonEnum(int editDistance, BytesRef lastTerm)
		...
		if (editDistance < runAutomata.size()) {
			...
			// System.out.println("FuzzyTE.getAEnum: ed=" + editDistance +
			// " lastTerm=" + (lastTerm==null ? "null" :
			// lastTerm.utf8ToString()));
			final CompiledAutomaton compiled = runAutomata.get(editDistance);
			return new AutomatonFuzzyTermsEnum(terms.intersect(
					compiled,
					lastTerm == null ? null : compiled.floor(lastTerm,
							new BytesRef())), runAutomata.subList(0,
					editDistance + 1).toArray(
					new CompiledAutomaton[editDistance + 1]));
		} else {
			return null;
		}
	}
		...
		if (runAutomata.size() <= maxDistance
			...
			LevenshteinAutomata builder = new LevenshteinAutomata(
					UnicodeUtil.newString(termText, realPrefixLength,
							termText.length - realPrefixLength), transpositions);
			...
			for (int i = runAutomata.size(); i <= maxDistance; i++) {
				...
				if (realPrefixLength > 0) {
					Automaton prefix = BasicAutomata.makeString(UnicodeUtil
							.newString(termText, 0, realPrefixLength));
	...
	private void bottomChanged(BytesRef lastTerm, boolean init)
		...
		boolean termAfter = bottomTerm == null
				|| (lastTerm != null && termComparator.compare(lastTerm,
						bottomTerm) >= 0);

		// as long as the max non-competitive boost is >= the max boost
		// for some edit distance, keep dropping the max edit distance.
		while (maxEdits > 0
				&& (termAfter ? bottom >= calculateMaxBoost(maxEdits)
	...
	protected void maxEditDistanceChanged(BytesRef lastTerm, int maxEdits,
			boolean init) throws IOException {
		TermsEnum newEnum = getAutomatonEnum(maxEdits, lastTerm);
		// instead of assert, we do a hard check in case someone uses our enum
		// directly
		// assert newEnum != null;
		if (newEnum == null) {
			assert maxEdits > LevenshteinAutomata.MAXIMUM_SUPPORTED_DISTANCE;
			throw new IllegalArgumentException(
					"maxEdits cannot be > LevenshteinAutomata.MAXIMUM_SUPPORTED_DISTANCE");
		}
		setEnum(newEnum);
	}
	...
	private int initialMaxDistance(float minimumSimilarity, int termLen) {
		// USERCODE_BEGIN this seems to be wrong since 0.6 of 5 is 3, which means 2 edits
		// and calculated edit distance was 1 (why?)
		//return (int) ((1D - minimumSimilarity) * termLen);
		return (int) (termLen - (minimumSimilarity * termLen));
	}
	 ...
	 * <p>
	 * This is the fastest method as opposed to LinearFuzzyTermsEnum: as
	 * enumeration is logarithmic to the number of terms (instead of linear) and
	 * comparison is linear to length of the term (rather than quadratic)
	 */
	private class AutomatonFuzzyTermsEnum extends FilteredTermsEnum {
		...
		public AutomatonFuzzyTermsEnum(TermsEnum tenum,
				CompiledAutomaton compiled[]) throws IOException {
			super(tenum, false);
		...
		protected AcceptStatus accept(BytesRef term) {
			//System.out.println("AFTE.accept term=" + term.utf8ToString());
			int ed = matchers.length - 1;

			// we are wrapping either an intersect() TermsEnum or an
			// AutomatonTermsENum,
			...
			// System.out.println("CHECK term=" + term.utf8ToString() + " ed=" +
			// ed);
			...
			if (ed == 0) { // exact match
				...
				// USERCODE_BEGIN cn: We better calculate the similarity by the offset of the request term. 
				// This seems clearer to me, than taking the smallest term
				//final int codePointCount = UnicodeUtil.codePointCount(term);
				//final float similarity = 1.0f - ((float) ed / (float) (Math
				//		.min(codePointCount, termLength)));
				//System.out.println("ed=" + ed + " codePointCount=" + codePointCount + " termLength=" + termLength);
				final float similarity = 1.0f - ((float) ed / (float) termLength);
				// USERCODE_BEGIN equal is also within range
				//if (similarity > minSimilarity) {
				if (similarity >= minSimilarity) {
					// USERCODE_BEGIN modified similarity gives better boost factor
					// boostAtt.setBoost((similarity - minSimilarity) *
					// scale_factor);
					boostAtt.setBoost(similarity);
					// USERCODE_END
					//System.out.println(similarity + "  yes");
					return AcceptStatus.YES;
				} else {
					//System.out.println(similarity + "  no");
		...
		final boolean matches(BytesRef term, int k) {
			return k == 0 ? term.equals(termRef) : matchers[k].run(term.bytes,
	...
	public static final class LevenshteinAutomataAttributeImpl extends
		...
		public boolean equals(Object other) {
			...
			if (!(other instanceof LevenshteinAutomataAttributeImpl))
				return false;
			return automata
					.equals(((LevenshteinAutomataAttributeImpl) other).automata);
		}

		@Override
		public void copyTo(AttributeImpl target) {
			final List<CompiledAutomaton> targetAutomata = ((LevenshteinAutomataAttribute) target)
					.automata();
